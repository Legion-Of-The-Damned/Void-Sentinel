...
class GeneralCommands(commands.Cog):
    ...
    async def help_command(self, interaction):
        embed = discord.Embed(
            ...
            description=(
                ...
                ":two::seven: `/выходной` — объявить выходной день (КВ отменяется)\n"
            ),
    ...
    async def ping(self, interaction):
        ...
        cpu_usage_percent = process.cpu_percent(interval=0.5)  # ТОЛЬКО нагрузка бота
        ...
        embed.add_field(name="CPU (бот)", value=f"{cpu_usage_percent:.1f}%", inline=True)
