...
class ClanWarNotifications(commands.Cog):
    def __init__(self, bot):
        ...
        self.off_days = set()
        self.off_days_file = "data/off_days.json"
    ...
    # ------------------- Выходные дни -------------------
    def load_off_days(self):
        try:
            if not os.path.exists(self.off_days_file):
                self.off_days = set()
                return

            with open(self.off_days_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                self.off_days = set(data)

        except Exception as e:
            logger.error(f"❌ ошибка загрузки выходных: {e}")
            self.off_days = set()

    def save_off_days(self):
        try:
            os.makedirs(os.path.dirname(self.off_days_file), exist_ok=True)
            with open(self.off_days_file, "w", encoding="utf-8") as f:
                json.dump(list(self.off_days), f, indent=2, ensure_ascii=False)

        except Exception as e:
            logger.error(f"❌ ошибка сохранения выходных: {e}")

    def cleanup_old_off_days(self):
        today = datetime.now().strftime("%Y-%m-%d")
        old = {d for d in self.off_days if d < today}
        if old:
            self.off_days -= old
            self.save_off_days()

    # ------------------- Команда /выходной -------------------
    @app_commands.command(name="выходной", description="Объявить выходной день (КВ отменяется)")
    @app_commands.describe(
        дата="Дата в формате ГГГГ-ММ-ДД (по умолчанию сегодня)",
        причина="Причина отмены (по умолчанию: Выходной)"
    )
    async def off_day_command(
        self,
        interaction: discord.Interaction,
        дата: str = None,
        причина: str = "Выходной"
    ):
        # Проверка права на управление ролями
        if not interaction.user.guild_permissions.manage_roles:
            await interaction.response.send_message(
                "❌ У вас недостаточно прав для использования этой команды. Требуется право «Управлять ролями».",
                ephemeral=True
            )
            return
        ...
        if target_date in self.off_days:
            await interaction.response.send_message(
                f"ℹ️ Дата {target_date} уже объявлена выходным.",
                ephemeral=True
            )
            return

        self.off_days.add(target_date)
        self.save_off_days()

        channel = self.bot.get_channel(self.announcements_channel_id)
        if channel:
            embed = discord.Embed(
                title="📢 Объявлен выходной!",
                description=f"**Дата:** {target_date}\n**Причина:** {причина}\n\nКлановые битвы в этот день **ОТМЕНЯЮТСЯ**.",
                color=discord.Color.orange()
            )
            embed.set_footer(text="Команда: /выходной")
            await channel.send(content=f"<@&{self.role_id}>", embed=embed)
        else:
            logger.warning(f"Канал {self.announcements_channel_id} не найден")

        await interaction.response.send_message(
            f"✅ Выходной день на {target_date} объявлен. Причина: {причина}",
            ephemeral=True
        )
    ...
    async def daily_announcement(self):
        ...
        # Если выходной — отправляем сообщение об отмене
        if cw_id in self.off_days:
            embed = discord.Embed(
                title="📢 Сегодня выходной!",
                description="Клановые битвы на сегодня **ОТМЕНЕНЫ**.",
                color=discord.Color.orange()
            )
            await channel.send(content=f"<@&{self.role_id}>", embed=embed)
        ...
        weekday = now.weekday()
        status = "🟥 Обязательное КВ" if weekday in [0, 1, 2, 6] else "🟩 Необязательное КВ"
